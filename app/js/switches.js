      ...
      $scope.$on('nodesDone', function () {
        ...
        for (let nid in nodes) {
          ...
          .then(function (resp) {
            ...
            let node = $scope._nodes[value.node_id];
            if (typeof value.value.lldp === 'undefined' ) {
              console.log('switches: missing lldp data for node ' +
                node.id + ' using fake data for testing');
              value.value = $scope.fakeValue(node.id);
            }
            for (let nic in value.value.lldp) {
              let sw = value.value.lldp[nic].chassis.mac;
              let raw = value.value.lldp[nic].port.descr;
              let portRegex = raw.match(/([0-9]*)\/([0-9]*)\/([0-9]*)/);
              let port = parseInt(portRegex[3]);
              //console.log(sw);
              //console.log(nic);
              //console.log(port);
              ...
              let nodemap = { id: node.id, name: node.name, nic: nic };
          ...
          'lldp': {
            'eth0': {
              ...
              'chassis': {
                ...
                'descr': 'Juniper Networks, Inc. qfx5100-24q-2p Ethernet ' +
                  'Switch, kernel JUNOS 13.2X51-D38, Build date: 2015-06-12' +
                  ' 02:33:47 UTC Copyright (c) 1996-2015 Juniper Networks, ' +
                  'Inc.',
                'mac': ( node_id % 2 === 0 ?
                  '20:4e:71:32:ca:fe' : '20:4e:71:32:be:ef'),
            ...
            'eth1': {
              ...
              'chassis': {
                ...
                'descr': 'Juniper Networks, Inc. qfx5100-24q-2p Ethernet ' +
                  ...
                  'Inc.',
                'mac': ( node_id % 2 === 0 ?
                  '20:4e:71:32:be:ef' : '20:4e:71:32:ca:fe'),
                'mgmt-ip': '192.168.105.2',
                'ttl': '120'
              },
              'port': {
